...
public class Vehicle {
    ...
    //added by me
    private boolean isStoppedInSegment;
    private int stopTimeRemainingInSegment;
    //private  originalColor;  // Store the original color of the vehicle
    ...
    public boolean isStoppedInSegment() {
        return isStoppedInSegment;
    }

    public void stopInSegment(int stopTime) {
        this.isStoppedInSegment = true;
        this.stopTimeRemainingInSegment = stopTime;
    }

    public void resumeMovementInSegment() {
        this.isStoppedInSegment = false;
        this.stopTimeRemainingInSegment = 0;
    }

    public void decrementStopTimeInSegment() {
        if (stopTimeRemainingInSegment > 0) {
            stopTimeRemainingInSegment--;
            if (stopTimeRemainingInSegment == 0) {
                resumeMovementInSegment();  // Resume movement when stop time is over
                color = originalcolor;
            }
        }
    }
...
//        double v_a = getSpeedForAcceleration();
    ...
    void moveVehicleInSegment() {
        if (type != PEDESTRIANS_ALONG_THE_ROAD_TYPE) {
            ...
            // Check if the vehicle is currently stopped in the segment
            if (isStoppedInSegment()) {
                decrementStopTimeInSegment();  // Decrement stop time and resume movement if time is up
                return;  // Skip further processing if the vehicle is stopped
            }
            // Randomly decide to stop at this place to pick up passengers
            //only bus will be stopped for pickup
            if (Utilities.randInt(0, 100) < Constants.STOP_PROBABILITY_IN_SEGMENT && (type==8 || type==9)) {
                stopInSegment(Constants.PICKUP_TIME_IN_SEGMENT);  // Stop for a predefined pickup time
                //when a vehicle stops then the color of thre vehicle change to red for the time it stops
                color = Color.RED;
                return;  // Vehicle stops, so no movement this step
            }
    
            // Existing logic for moving the vehicle in the segment
            if (isObjectInProximity() || !moveForwardInSegment() || isSlowerVehicleInProximity()) {
                tryLaneChange(true);
            }
    
        } else {
            // Existing pedestrian handling logic...
            if (Utilities.randInt(0, 101) < Parameters.PEDESTRIAN_RANDOM_LANE_CHANGE_PERCENTAGE) {
                // Pedestrian-specific lane change logic...
...
//                    case GHR_MODEL -> getAccelerationGHRModel(leader, this) < 0;
    ...
    private boolean moveForwardInSegment() {
        ...
        if (isStoppedInSegment()) {
            decrementStopTimeInSegment();  // Decrement stop time and resume movement if time is up
            return false;  // Vehicle is stopped, so it cannot move forward
        }
    
        // Randomly decide to stop at this place to pick up passengers
        if (shouldStopInSegment()) {  // New method to randomly decide the stop
            stopInSegment(Constants.PICKUP_TIME_IN_SEGMENT);  // Stop for a predefined pickup time
            return false;  // Vehicle stops, so no movement this step
        }
    
        if (controlSpeedInSegment()) {
            storePrevGaps();
            distanceInSegment = getNewDistanceInSegment();
            if (distanceInSegment > segment.getLength() - length) { 
                distanceInSegment = segment.getLength() - length - (MARGIN - 0.1);
            }
            if (!passedSensor) {
            ...
            return true;  // Vehicle moves forward
        } else {
            waitingTime++;
            waitingTimeInSegment++;
            return false;  // Vehicle cannot move forward due to speed control or other factors
        }
    }
    
    // Method to randomly decide whether to stop in the segment
    private boolean shouldStopInSegment() {
        int randomValue = Utilities.randInt(0, 100);
        return randomValue < Constants.STOP_PROBABILITY_IN_SEGMENT;  // STOP_PROBABILITY_IN_SEGMENT is the probability (e.g., 5%) to stop
    }
